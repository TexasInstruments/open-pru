...
2. Work through the Decision tree and record your answers — D1–D3, the
   Npru/N-hw/N-sw/I/O findings, the section it routes you to, and the Do-not-do
   constraints — in a "Run parameters" note you keep visible. Later sections
   branch on the answers; V6 checks the constraints held.
3. Turn each `- [ ]` item in the section your answers select into tracked
   work:
   ...
   items or the pasted checklist — so tracking is visible before any step runs.
4. A checkbox with sub-bullets is done only when every sub-bullet is done.
5. Do the steps in order; later steps depend on earlier outputs. Do not skip an
   item because it "looks unnecessary." If an item is N/A for your port, mark
   it N/A with a one-line reason.
...
→ Record your D1–D3 answers and the Npru/N-hw/N-sw/I/O findings before
  proceeding to Section A or B.
...
- [ ] A6. If the project has MCU+ code, repeat A2–A5 for the MCU+ directory.
  ...
  `mcuplus/<new_board>/<core>/ti-arm-clang/` (for example, `examples/empty`);
  others use `<new_board>/<core>/ti-arm-clang/` directly at the project root
  (for example, `academy/intc/intc_mcu`). Match the convention used by the project
  being ported.
...
- [ ] C1. Present the new board/device tree and the renamed CCS project names,
  and confirm they match your recorded scope — the target device/board, and the
  core mapping (D2, and B4 for a device port). Fix any naming or mapping error
  now; it propagates into every makefile and projectspec and is cheapest to fix
...
- [ ] V1. Build PRU firmware from the core directory:
  ...
  Paste the actual command output. Expected: no errors; `.out` generated;
  `*_load_bin.h` generated if the project has a MCU+ host (not produced for
  AM62x Linux-host projects).
- [ ] V2. Build from the project directory:
  `cd <project> && make -s clean && make`
  Paste the actual command output. Expected: no errors.
- [ ] V3. Build from the repo top level:
  `cd <repo_root> && make -s`
  Paste the actual command output. Expected: new board/device builds without
  errors, and the already-supported devices still build.
- [ ] V4. Import and build in CCS. Paste the CCS build console output.
  Expected: `example.projectspec` opens without errors and builds PRU firmware
  and R5F (if present).
...
- [ ] V6. Confirm each Do-not-do constraint recorded in your run parameters
  held. State the outcome for each — a result that builds can still violate one.
...
- Do not retry a failing build indefinitely or invent a workaround. After a few
  fix attempts, stop and report the failure with its output.
