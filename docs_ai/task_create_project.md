...
2. Answer D1–D3 and record the answers — plus the Do-not-do constraints — in a
   "Run parameters" note you keep visible. Every section below branches on the
   answers; V5 checks the constraints held.
3. Turn each `- [ ]` item in the sections your D1–D3 answers select into
   tracked work:
...
5. Do the steps in order; later sections depend on earlier outputs. Do not skip
   an item because it "looks unnecessary." If an item is N/A given your D1–D3
...
→ Record your D1, D2, and D3 answers before proceeding to Section A.
...
- [ ] A5. Checkpoint before customizing. Present the copied, renamed skeleton
  (the directory layout and the renamed makefile/projectspec names) and confirm
  it matches your recorded D1/D2 scope — the intended host directories, device
  and board list, and cores. Fix any interpretation error now, before Section
  B; a wrong skeleton propagates into every step that follows.
...
- [ ] V1. From the project directory: `make -s clean && make`
  Paste the actual command output (not a summary). Expected: no errors;
  all expected output files generated.
- [ ] V2. From the repo root: `make -s clean && make -s`
  Paste the actual command output. Expected: new project is included and
  builds without errors.
- [ ] V3. In CCS: import `example.projectspec`, build PRU firmware first,
  then R5F (if present). Paste the CCS build console output. Expected: no
  errors or warnings about missing files.
- [ ] V4. Confirm `README.md` covers all items in Section D1. List the
  headings present so the coverage is visible.
- [ ] V5. Confirm each Do-not-do constraint recorded in your run parameters
